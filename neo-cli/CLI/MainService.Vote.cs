...
namespace Neo.CLI
{
    partial class MainService
    {
        ...
        {
            var testGas = NativeContract.DVITA.GetRegisterPrice(NeoSystem.StoreView) + (BigInteger)Math.Pow(10, NativeContract.GAS.Decimals) * 10;
            if (NoWallet()) return;
            ...
            {
                scriptBuilder.EmitDynamicCall(NativeContract.DVITA.Hash, "registerCandidate", publicKey);
        ...
        private void OnUnregisterCandidateCommand(UInt160 account)
        {
            ...
            using (ScriptBuilder scriptBuilder = new ScriptBuilder())
            {
                scriptBuilder.EmitDynamicCall(NativeContract.DVITA.Hash, "unregisterCandidate", publicKey);
                script = scriptBuilder.ToArray();
            }
        ...
        private void OnVoteCommand(UInt160 senderAccount, ECPoint publicKey)
        {
            ...
            using (ScriptBuilder scriptBuilder = new ScriptBuilder())
            {
                scriptBuilder.EmitDynamicCall(NativeContract.DVITA.Hash, "vote", senderAccount, publicKey);
                script = scriptBuilder.ToArray();
            }
        ...
        private void OnUnvoteCommand(UInt160 senderAccount)
        {
            ...
            using (ScriptBuilder scriptBuilder = new ScriptBuilder())
            {
                scriptBuilder.EmitDynamicCall(NativeContract.DVITA.Hash, "vote", senderAccount, null);
                script = scriptBuilder.ToArray();
            }
        ...
        {
            if (!OnInvokeWithResult(NativeContract.DVITA.Hash, "getCandidates", out StackItem result, null, null, false)) return;
        ...
        {
            if (!OnInvokeWithResult(NativeContract.DVITA.Hash, "getCommittee", out StackItem result, null, null, false)) return;
        ...
        {
            if (!OnInvokeWithResult(NativeContract.DVITA.Hash, "getNextBlockValidators", out StackItem result, null, null, false)) return;
        ...
        private void OnGetAccountState(UInt160 address)
        {
            ...
            if (!OnInvokeWithResult(NativeContract.DVITA.Hash, "getAccountState", out StackItem result, null, new JArray(arg))) return;
            Console.WriteLine();
            if (result.IsNull)
            {
