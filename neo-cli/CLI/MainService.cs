...
namespace Neo.CLI
{
    public partial class MainService : ConsoleServiceBase, IWalletProvider
    {
        ...
        // TODO: to Weak refs
        private static MainService instanceRef;
        ...
        protected override string Prompt => "nxa";
        public override string ServiceName => "NXA-CLI";
        ...
        {
            RegisterCommandHander<string, byte[]>(false, (str) => StringToByteArray(str));
        ...
        //  Converts string -> byte[] since string -> string isn't possible.
        //
        internal static byte[] StringToByteArray(string str) {
            // if (!Helper.IsSocialHandle(str)) {
            //     return null;
            // }
        ...
        internal static UInt160 StringToAddress(string input, byte version) {
            ...
            {
                case "dvita": return NativeContract.DVITA.Hash;
                case "dvg": return NativeContract.GAS.Hash;
            ...
            // dVITA Naming Service

            if (Helper.IsCNRAddress(input))
            {
                var resolved = ResolveAddressByCNR(input);
            ...
            var scriptHash = input.ToScriptHash(version);
        ...
        public override void RunConsole()
        {
            ...
            Console.WriteLine($"{ServiceName} v{cliV}  -  NXA v{neoV}  -  NXA-VM v{vmV}");
            Console.WriteLine();
        ...
        public async void Start(string[] args)
        {
            ...
            if (Settings.Default.UnlockWallet.IsActive)
            {
                ...
                {
                    ConsoleHelper.Error($"{ex.GetBaseException().Message}: {ex.ToString()}");
                }
            }
        }
