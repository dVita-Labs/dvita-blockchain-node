    ...
    partial class MainService
    {
        ...
        private void OnListAssetCommand()
        {
            ...
            foreach (UInt160 account in CurrentWallet.GetAccounts().Select(p => p.ScriptHash))
            {
                ...
                ConsoleHelper.Info("DVITA: ", $"{CurrentWallet.GetBalance(snapshot, NativeContract.DVITA.Hash, account)}");
                ConsoleHelper.Info("DVG:   ", $"{CurrentWallet.GetBalance(snapshot, NativeContract.GAS.Hash, account)}");
                Console.WriteLine();
            }
            Console.WriteLine("----------------------------------------------------");
            ConsoleHelper.Info("Total:   DVITA: ", $"{CurrentWallet.GetAvailable(snapshot, NativeContract.DVITA.Hash),10}     ", "DVG: ", $"{CurrentWallet.GetAvailable(snapshot, NativeContract.GAS.Hash),18}");
            Console.WriteLine();
            ConsoleHelper.Info("DVITA hash: ", NativeContract.DVITA.Hash.ToString());
            ConsoleHelper.Info("DVG hash:   ", NativeContract.GAS.Hash.ToString());
        }
